...
import { FORMAT_DATE, MSG, ROLE, SCREEN_MODE, STATUS, STATUS_KEY_STRING, TOOLTIP } from '../../core/configs/constants';
import { useStore, useTitle } from '../../core/utils/hook';
import SettingAdminScreen from './SettingAdminScreen';
import { useTranslation } from 'react-i18next';
import classNames from 'classnames';

const AdminScreen = observer(props => {

    // translation
    const { t, i18n: { language } } = useTranslation();

    // other
    useTitle(t('adminScreen.titleDocument'));

    // store
    const { 
        permissionStore: { permissionsList, getAllPermissions },
        userStore: { clean, getAllUser, adminList, paging, setAttrObservable, updateUser, deleteUser },
    ...
    useEffect(() => {
        clean();
        getAllPermissions({ isSearch: true });
        onFetchData();
    ...
    const onShowSettingAdmin = (mode, id) => {
        show({
            id: 'modal-setting-admin',
            isOpen: true,
            header: mode === SCREEN_MODE.ADD ? t('settingAdminScreen.titleCreateAdmin') : t('settingAdminScreen.titleUpdateAdmin'),
            onCancel: () => hide(),
            children: (
                <SettingAdminScreen mode={mode} id={id}/>
            ),
            type: 'medium'
        })
    }
    ...
    const onShowConfirmChangeStatus = (id, status) => {
        const msg = status === STATUS_KEY_STRING.INACTIVE ? t('adminScreen.confirmInactiveAdmin') : t('adminScreen.confirmActiveAdmin');
        openWarningModal(msg, () => onSubmitUpdate(1, id, { status }));
    }

    const onShowConfirmDeleteAccount = (id) => {
        openWarningModal(t('adminScreen.confirmDeleteAdmin'), () => onSubmitUpdate(2, id));
    }
    ...
    const columns = [
        ...
        {
            Header: t('adminScreen.permission'),
    ...
    return(
        <div className='admin-screen'>
            <div className='container-title'>{t('adminScreen.titleScreen')}</div>
            <div className='container-search'>
                <form onSubmit={handleSubmit(onSearch)}>
                    <div className='row'>
                        <div className='row col-6 align-items-center'>
                            <label className='col-2-5'>{t('adminScreen.fullName')}</label>
                            <input {...register('name')} className='col-7' />
                        </div>
                        <div className='row col-6 align-items-center justify-content-end align-items-center'>
                            <label className='col-2-5'>{t('adminScreen.permission')}</label>
                            <select {...register('permissionId')} className='col-7'>
                                <option value={''}>{t('common.all')}</option>
                                {
                                    permissionsList?.length > 0 &&
                                    permissionsList.map(e => <option key={e.id} value={e._id}>{e.name}</option>)
                                }
                            </select>
                        </div>
                    </div>
                    <div className='row mg-t-20'>
                        <div className='row col-6'>
                            <label className='col-2-5'>{t('adminScreen.email')}</label>
