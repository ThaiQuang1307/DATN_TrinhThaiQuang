            ...
            <div className='mg-t-80 d-flex justify-content-center align-items-center flex-column'>
                {
                    confirmEmailStatus === CONFIRM_EMAIL_STATUS.CONFIRM_EMAIL_SUCCESS ?
                        <i className='fas fa-check-circle text-success font-size-70'></i>
                        :
                        <i className='fas fa-times-circle text-danger font-size-70'></i>
                }
                <div className='mg-t-50 fw-bolder pd-lr-20'>{CONFIRM_EMAIL_STATUS_STRING[confirmEmailStatus]}</div>
                {
                    confirmEmailStatus === CONFIRM_EMAIL_STATUS.CONFIRM_EMAIL_SUCCESS ? 
                        <button className='mg-t-50 btn btn-primary py-2 px-4' onClick={() => navigate(SYSTEM_PATH.LOGIN)}>Go to log in</button>
                        :
                        <button className='mg-t-50 btn btn-primary py-2 px-4' onClick={() => navigate(SYSTEM_PATH.HOME)}>Go to home page</button>
                }
            </div>
        </div>
    );
